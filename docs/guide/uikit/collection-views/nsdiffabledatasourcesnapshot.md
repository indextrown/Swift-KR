...
Diffable Data Source는 snapshot으로 Collection View와 Table View에 데이터를 제공해요. 최초로 화면에 표시할 데이터 상태를 구성할 때도 snapshot을 사용하고, 표시 중인 데이터가 달라졌음을 화면에 반영할 때도 새 snapshot을 사용해요.

snapshot에는 화면에 표시할 section과 item이 개발자가 정한 순서대로 들어 있어요. section과 item을 추가·삭제·이동해 원하는 표시 상태를 구성해요.

> **중요:** 모든 section과 item은 `Hashable`을 준수하는 고유 식별자를 가져야 해요. `struct`나 `enum`, 또는 `Int`, `String`, `UUID` 같은 Swift 값 타입을 식별자로 사용하는 것이 좋아요. class를 식별자로 사용해야 한다면 `NSObject`의 하위 클래스여야 하며, 변경 가능한 값 때문에 hash나 동등성 결과가 달라지지 않도록 주의해야 해요.

snapshot으로 데이터를 표시하는 순서는 다음과 같아요.

1. snapshot을 만들고 화면에 표시하려는 현재 데이터 상태로 채워요.
2. snapshot을 Diffable Data Source에 적용해 변경 결과를 사용자 인터페이스에 반영해요.

snapshot은 다음 두 방식으로 만들 수 있어요.

- 빈 snapshot을 만든 뒤 section과 item을 차례대로 추가해요.
- Diffable Data Source의 `snapshot()`으로 현재 snapshot을 가져온 뒤, 새 데이터 상태에 맞게 수정해요.

다음 코드는 빈 snapshot에 section 하나와 item 세 개를 추가한 뒤, 이전 상태와 새 상태의 차이를 애니메이션으로 적용해요.
...
같은 snapshot 타입은 다음 Diffable Data Source에서 사용할 수 있어요.
...
Objective-C의 `NSDiffableDataSourceSnapshotReference` 객체를 Swift의 generic snapshot으로 변환할 수 있어요.
