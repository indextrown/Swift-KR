...
Section snapshot은 Collection View의 section 하나에 표시할 데이터를 표현해요. 개별 section에 처음 표시할 데이터 상태를 설정하고, 이후 그 section의 데이터만 갱신할 수 있어요.

화면 전체의 데이터를 나타내는 `NSDiffableDataSourceSnapshot`과 함께 사용하거나 section snapshot만 사용할 수 있어요. 여러 section이 서로 다른 출처에서 데이터를 받아 section별로 정밀하게 관리해야 할 때 유용해요. 펼치고 접을 수 있는 outline처럼 계층 구조를 가진 데이터도 section snapshot으로 표현할 수 있어요.

다음 예제는 두 개의 root item을 만들고, 그중 `Food` 아래에 세 개의 child item을 추가해 각 section에 적용해요.
