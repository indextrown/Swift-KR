...
### 데이터는 Data Source가 제공해요

앱의 주된 책임은 Collection View에 연결할 데이터를 관리하는 일이에요. 데이터는 item과 section으로 구성되고, item 하나는 `UICollectionViewCell` 하나로 표현돼요. 새 구현에서는 `UICollectionViewDiffableDataSource`로 snapshot을 적용할 수 있고, 전통적인 방식이 필요하면 `UICollectionViewDataSource`를 직접 구현할 수 있어요.

셀 이외에도 header와 footer 같은 supplementary view를 표시할 수 있어요. 어떤 supplementary view가 필요한지와 그 위치는 layout이 정하고, 실제 뷰와 내용은 data source가 제공해요.

### Layout은 보이는 뷰가 아니라 Attributes를 계산해요

Collection View는 layout 객체에 현재 영역에 필요한 attributes를 요청한 뒤 그 결과를 셀과 supplementary view에 적용해요. 데이터가 바뀌거나 bounds가 달라져 layout이 무효화되면 필요한 attributes를 다시 요청해요. layout을 바꿀 때는 즉시 교체하거나 애니메이션·대화형 전환을 사용할 수 있어요.

### Prefetch는 표시 전에 시작되고 취소될 수 있어요

`prefetchDataSource`를 연결하면 곧 보일 item의 데이터를 미리 준비할 수 있어요. 이 요청은 힌트이므로 순서대로 오거나 반드시 셀이 표시된다는 보장이 없고, `cancelPrefetchingForItemsAt`에서 불필요한 작업을 취소해야 해요.

### 대화형 재배치는 모델까지 갱신해야 해요

`beginInteractiveMovementForItem(at:)`으로 시작한 이동은 화면 좌표를 따라가지만, 최종 순서는 data source에도 반영해야 유지돼요. Drag and Drop API를 사용하면 앱 내부 이동뿐 아니라 다른 앱과의 데이터 전달도 처리할 수 있어요.

### 국제화와 접근성을 함께 확인해요

Flow Layout과 Compositional Layout에서는 `leading`·`trailing`을 사용해 오른쪽에서 왼쪽으로 읽는 언어에도 대응해요. 셀의 의미, 선택 상태, 재배치 가능 여부가 VoiceOver에 전달되는지도 확인하고, 단순히 시각적 위치만 바꾸지 않도록 주의하세요.
