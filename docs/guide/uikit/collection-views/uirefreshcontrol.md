...
`UIRefreshControl`은 Table View와 Collection View를 포함한 모든 `UIScrollView` 객체에 연결할 수 있는 표준 새로고침 control이에요. 사용자가 스크롤 가능한 콘텐츠의 맨 위를 아래로 끌면 Scroll View가 Refresh Control을 드러내고 진행 표시 애니메이션을 시작한 뒤 앱에 새로고침을 알려요. 앱은 이 알림을 받아 콘텐츠를 갱신하고 Refresh Control을 닫아요.
...
Refresh Control은 `UIControl`의 target-action 방식으로 콘텐츠를 갱신할 시점을 전달해요. 구성할 때 `.valueChanged` 이벤트에 action 메서드를 연결하고, action에서 콘텐츠를 갱신한 다음 작업이 끝나면 `endRefreshing()`을 호출해요.
...
`UITableViewController`를 사용한다면 컨트롤러의 `refreshControl` 프로퍼티에 `UIRefreshControl` 인스턴스를 대입해요. 그런 다음 `.valueChanged` 이벤트의 target과 action을 연결해 Table View의 새로고침 동작을 처리해요.
