...
Supplementary Item은 콘텐츠에 추가 뷰를 붙일 때 사용해요. item에 badge를 붙이거나 group 주위에 frame을 표시하는 경우가 예예요. Supplementary Item은 연결된 item의 IndexPath를 따라가요.

layout이나 section의 header 또는 footer를 만들려면 일반 Supplementary Item 대신 `NSCollectionLayoutBoundarySupplementaryItem`을 사용해요.

Supplementary Item 종류마다 고유한 element kind가 필요해요. 각 문자열을 쉽게 구분하고 같은 곳에서 관리할 수 있도록 다음처럼 모아 둘 수 있어요.
...
item을 만들 때 Supplementary Item 배열을 전달하면 해당 item에 보조 뷰를 연결할 수 있어요.
