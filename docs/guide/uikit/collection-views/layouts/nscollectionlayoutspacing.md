...
`.fixed(12)`는 item 사이에 정확히 12 point를 유지해요. `.flexible(12)`는 최소 12 point를 확보하고 남는 공간이 있으면 item 사이에 나눠 간격을 늘릴 수 있어요.
...
`interItemSpacing`은 형제 item 사이, edge spacing은 요소 바깥 경계, `contentInsets`는 요소 내부 콘텐츠 영역을 조정해요. 세 개념을 섞으면 계산된 크기가 예상보다 작아질 수 있으므로 어느 수준의 여백인지 구분하세요.
