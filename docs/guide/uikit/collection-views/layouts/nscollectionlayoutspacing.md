...
Collection View Layout에서는 spacing 객체로 간격의 크기와 그 간격을 계산하는 방식을 함께 지정해요.

spacing은 fixed 또는 flexible 방식으로 표현할 수 있어요.

**Fixed spacing**은 정확한 간격이 필요할 때 사용해요. 다음 코드는 group 안의 item 사이에 항상 200 point를 둬요.

```swift
group.interItemSpacing = .fixed(200.0)
```

**Flexible spacing**은 최소 간격을 보장하면서 남는 공간이 생기면 더 늘어날 수 있게 해요. 다음 코드는 item 사이에 최소 200 point를 확보하고, 추가 공간이 있으면 item을 고르게 다시 배치해 간격을 늘려요.
...
| API            | 하는 일                                               |
| -------------- | ----------------------------------------------------- |
| `fixed(_:)`    | 늘어나지 않는 정확한 간격을 만들어요.                 |
| `flexible(_:)` | 남는 공간에 따라 늘어날 수 있는 최소 간격을 만들어요. |
...
| API       | 하는 일                                       |
| --------- | --------------------------------------------- |
| `spacing` | fixed 또는 flexible spacing의 point 값이에요. |
...
| API          | 하는 일                                            |
| ------------ | -------------------------------------------------- |
| `isFixed`    | 늘어나지 않는 고정 간격인지 나타내요.              |
| `isFlexible` | 남는 공간에 따라 늘어날 수 있는 간격인지 나타내요. |
