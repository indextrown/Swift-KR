...
Collection View Layout의 각 section에는 Visible Items Invalidation Handler를 지정할 수 있어요. 이 handler는 현재 section bounds 안에 보이는 item에 사용자 정의 애니메이션을 적용할 때 사용해요. item 추가·삭제, section 스크롤, 기기 회전처럼 section에서 애니메이션이나 layout 변화가 발생하면 각 layout cycle 직전에 호출돼요.
