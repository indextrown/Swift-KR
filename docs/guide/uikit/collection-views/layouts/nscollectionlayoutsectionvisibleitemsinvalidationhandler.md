...
각 section은 visible item invalidation handler를 하나 가질 수 있어요. item 삽입·삭제 애니메이션, 스크롤, 회전처럼 section의 layout cycle이 일어날 때 보이는 item 배열과 content offset, environment를 받아요.

```swift
section.visibleItemsInvalidationHandler = {
  visibleItems, contentOffset, environment in
  let center =
    contentOffset.x + environment.container.effectiveContentSize.width / 2
  for item in visibleItems {
    let distance = abs(item.center.x - center)
    item.transform = CGAffineTransform(
      scaleX: max(0.85, 1 - distance / 1_000),
      y: max(0.85, 1 - distance / 1_000)
    )
  }
}
```

handler는 스크롤 중 자주 호출되므로 네트워크 요청, 이미지 디코딩, 복잡한 검색을 하지 말고 현재 visible item의 `alpha`, `transform`, `zIndex`처럼 표시 속성만 빠르게 바꾸세요.
