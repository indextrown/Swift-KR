...
description: 'NSCollectionLayoutDecorationItem은 데이터 소스와 무관하게 레이아웃이 section 배경 같은 장식 뷰를 배치하도록 정의해요. 주요 API, 사용 예제, 주의점을 함께 정리합니다.'
---

# NSCollectionLayoutDecorationItem

> **면접 답변 한 줄 요약:** `NSCollectionLayoutDecorationItem`은 데이터 소스와 무관하게 레이아웃이 section 배경 같은 장식 뷰를 배치하도록 정의해요.
...
NSCollectionLayoutDecorationItem은 데이터 소스와 무관하게 레이아웃이 section 배경 같은 장식 뷰를 배치하도록 정의해요.

## 공식 설명에서 놓치면 안 되는 동작

Decoration View는 item 데이터와 연결되지 않은 section 배경 같은 시각 요소예요. 고유한 element kind를 정의하고 section에 decoration item을 추가한 뒤, Compositional Layout에 실제 뷰 클래스를 등록해야 해요.
...
data source가 supplementary view를 제공하는 것과 달리 Decoration View는 layout이 만들고 관리해요.
...
background.contentInsets = .init(top: 8, leading: 8, bottom: 8, trailing: 8)
background.zIndex = -1
section.decorationItems = [background]
```
...
### background 만들기 (Creating a background)

`NSCollectionLayoutDecorationItem`를 만들거나 필요한 구성 요소를 연결하는 API예요.

| API                        | 하는 일                                                |
| -------------------------- | ------------------------------------------------------ |
| `background(elementKind:)` | 지정한 kind의 section 배경 decoration item을 만들어요. |
...
- [Apple Developer Documentation — NSCollectionLayoutDecorationItem](https://developer.apple.com/documentation/uikit/nscollectionlayoutdecorationitem)
