...
원문: [Apple Developer Documentation — Layouts](https://developer.apple.com/documentation/uikit/layouts)
...
Collection View Layout은 Collection View 콘텐츠의 시각적 배치를 정의해요. Layout은 단순한 배치부터 복잡한 배치까지 어떤 형태든 만들 수 있도록 유연하게 설계되어 있어요. 가장 단순한 Collection View Layout은 item을 grid로 표시하지만, 필요하다면 item을 원형으로 배치하거나 각 item의 크기를 다르게 만들 수도 있어요. 기기를 회전해 item을 다른 방식으로 보여 줘야 하는 경우처럼, 실행 중에도 필요에 따라 Layout을 동적으로 변경할 수 있어요.

Collection View Layout은 사용자 정의가 가능하고 결과가 시각적으로 드러나요. 예를 들어 App Store 앱은 하나의 Collection View 안에서 section마다 서로 다른 사용자 정의 Layout을 사용해 여러 종류의 콘텐츠를 보여 줘요.
...
디자인 지침은 Apple의 [Human Interface Guidelines](https://developer.apple.com/design/human-interface-guidelines)에서 확인할 수 있어요.

## Swift-KR 보충: 공식 API를 찾아봐요

아래 목차는 Apple의 Layouts 목차와 같은 순서를 사용해요. 먼저 [레이아웃 학습 가이드](../layout-guide)에서 item·group·section 조립 원리를 익힌 뒤 필요한 API의 전체 멤버를 확인할 수 있어요.
