...
boundary supplementary item은 section 또는 전체 layout의 경계에 header와 footer를 배치하는 특수한 supplementary item이에요. 종류별 element kind를 한곳에 모아 registration과 layout이 같은 문자열을 사용하게 하세요.
...
높이를 `.estimated(44)`로 지정하면 실제 텍스트와 Dynamic Type에 맞춰 측정할 수 있어요. `pinToVisibleBounds`는 스크롤 중 경계에 고정할지, `extendsBoundary`는 section 바깥까지 확장할지 정해요.
