...
이 타입은 `UICollectionViewDataSource`를 이미 채택하고 필수 메서드를 구현해요. 화면을 채우는 기본 순서는 다음과 같아요.

1. Collection View와 Diffable Data Source를 연결해요.
2. cell provider에서 item 식별자로 최신 모델을 조회하고 셀을 구성해요.
3. section과 item 식별자가 담긴 snapshot을 만들어요.
4. `apply(_:animatingDifferences:)`로 snapshot을 적용해요.

```swift
let dataSource = UICollectionViewDiffableDataSource<Section, Photo.ID>(
  collectionView: collectionView
) { collectionView, indexPath, photoID in
  guard let photo = photoStore[photoID] else { return nil }
...
> **중요:** `apply`가 완료되기 전에 같은 data source에 서로 의존하는 snapshot을 무질서하게 적용하지 마세요. UI 관련 구성과 적용은 main actor에서 수행하고, 식별자는 snapshot 안에서 고유해야 해요.

Supplementary View는 `supplementaryViewProvider`로 구성하고, 계층형 section은 `apply(_:to:animatingDifferences:)`에 `NSDiffableDataSourceSectionSnapshot`을 전달해요. `reorderingHandlers`는 재배치 가능 여부와 모델 반영을, `sectionSnapshotHandlers`는 펼침·접힘 이벤트를 처리해요.
