...
Data source 객체는 Collection View에 표시할 데이터를 관리해요. 앱의 데이터 모델을 대표하며, Collection View가 필요로 할 때 데이터에 관한 정보를 제공해요. 또한 Collection View가 데이터를 표시하는 데 사용할 셀과 supplementary view를 만들고 구성해요.

Collection View의 data source는 `UICollectionViewDataSource` 프로토콜을 준수해야 해요. 이 프로토콜을 이미 채택한 `UICollectionViewDiffableDataSource`를 data source로 사용할 수 있어요.

직접 data source를 만들려면 `UICollectionViewDataSource`를 채택해요. 모든 data source는 최소한 `collectionView(_:numberOfItemsInSection:)`과 `collectionView(_:cellForItemAt:)`을 구현해야 해요. 두 메서드는 Collection View에 있는 item 개수와 각 item을 표시할 셀을 제공해요. 나머지 메서드는 선택 사항이며, 여러 section을 구성하거나 section별 header와 footer를 표시할 때 구현해요.

Collection View를 구성할 때 만든 data source를 `dataSource` 프로퍼티에 연결해요. Collection View와 data source가 콘텐츠를 표시하기 위해 협력하는 과정은 `UICollectionView` 문서에서 확인할 수 있어요.
