...
Collection View에 placeholder cell을 넣으려면 `UICollectionViewDropPlaceholder` 객체를 만들고 `UICollectionViewDropCoordinator`의 `drop(_:to:)`에 전달해요. Placeholder Cell은 실제 콘텐츠를 비동기로 불러오는 동안 보여 줄 임시 인터페이스예요. 진행 표시나 아직 콘텐츠를 사용할 수 없다는 안내를 표시할 수 있어요. Placeholder 객체에는 임시 셀의 reuse identifier가 들어가며, 필요하면 drop 중 사용할 사용자 정의 preview도 포함할 수 있어요.

Placeholder에 사용할 셀은 미리 등록해야 해요. storyboard에 Collection View Cell을 추가해 모양과 class를 구성하고 reuse identifier를 지정하거나, 코드에서 같은 정보를 등록해요. `init(insertionIndexPath:reuseIdentifier:)`로 placeholder를 만들 때 그 reuse identifier를 전달하면 Collection View가 이 정보를 이용해 임시 셀을 삽입해요.

`cellUpdateHandler`에는 들어오는 데이터를 기다리는 동안 placeholder cell을 어떻게 구성할지 작성해요.

전체 비동기 drop 흐름은 [Collection View에서 드래그 앤 드롭 지원하기](./supporting-drag-and-drop-in-collection-views)에서 확인할 수 있어요.

> **중요:** Placeholder Cell은 Collection View에 잠깐만 존재해야 해요. 가능한 한 빨리 실제 셀로 교체하거나 drop을 취소해 제거하세요. `UICollectionViewDropPlaceholderContext`의 메서드로 placeholder를 최종 item으로 바꾸거나 삭제할 수 있어요.
