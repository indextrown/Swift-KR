...
| 캡처 목록      | 클로저가 바깥 값을 강하게, 약하게, 미소유 방식 중 어떻게 기억할지 정하는 `[...]` 문법이에요.                                   |
...
1. `showProfile()`을 실행할 기계어 명령은 코드 영역과 연결해 생각할 수 있어요.
2. 전역 변수 `launchCount`처럼 프로그램과 수명을 함께하는 저장소는 데이터 영역의 예로 볼 수 있어요.
3. 함수가 실행되는 동안 사용하는 `localID`와 `profile` 지역 변수는 스택 프레임에서 이해할 수 있어요.
4. `Profile` 클래스 인스턴스는 동적인 수명을 가지므로 힙 객체로 이해할 수 있어요.
5. 스택의 `profile`에는 객체 전체가 아니라 힙 객체를 가리키는 참조가 있다고 생각하면 돼요.

Apple 플랫폼의 실행 파일 형식인 Mach-O에는 실행 코드와 읽기 전용 데이터를 담는 `__TEXT`, 쓰기 가능한 데이터를 담는 `__DATA` 같은 세그먼트가 있어요. 이 구분은 Apple의 보관 문서인 [Overview of the Mach-O Executable Format](https://developer.apple.com/library/archive/documentation/Performance/Conceptual/CodeFootprint/Articles/MachOOverview.html)에서 확인할 수 있어요. 오래된 성능 문서이므로 세그먼트의 개념을 이해하는 참고 자료로만 사용해요.
...
## 캡처 목록의 weak self로 클로저 순환을 끊어요

클로저 시작 부분의 캡처 목록에 `[weak self]`를 작성하면 `self`를 소유하지 않고 기억해요.
...
캡처 목록은 습관적으로 붙이는 문법이 아니라 객체의 수명 계약을 표현하는 문법이에요.
