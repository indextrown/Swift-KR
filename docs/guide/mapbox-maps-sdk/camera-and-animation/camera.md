...
![지정한 좌표 경계 밖으로 카메라 이동을 제한한 지도](../assets/camera-bounds.png)

_붉은 경계는 사용자가 탐색할 수 있는 좌표 범위를 보여 주는 예시예요. 한 번 좌표를 화면에 맞추는 것과 지속적으로 이동 범위를 제한하는 것을 구분해요. [공식 Camera position에서 경계 제한 이미지 보기](https://docs.mapbox.com/ios/maps/guides/camera-and-animation/camera/)_
