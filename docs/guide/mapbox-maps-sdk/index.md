...
![Mapbox Standard 스타일을 서로 다른 지도 용도에 적용한 비교](./assets/standard-use-cases.png)

_같은 Standard 스타일도 탐색·운전·장소 안내처럼 목적에 따라 다른 화면을 구성할 수 있어요. [공식 가이드에서 이미지와 설명 보기](https://docs.mapbox.com/ios/maps/guides/)_
...
:::info 이미지 출처와 권리
이 섹션의 Mapbox 화면과 도식은 공식 가이드의 학습 맥락을 유지하기 위해 로컬 파일로 보관했어요. 각 이미지에는 원문 링크를 표시하며 이미지의 권리는 Mapbox에 있어요. 파일과 출처의 대응은 `assets/image-manifest.json`에서 확인할 수 있어요.
:::
